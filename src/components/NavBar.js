...
const NavBar = (props) => {
  ...
  const showForm = (closeManual = true) => (ev = null) => {
    ...
    const formEl = userFormRef.current ? userFormRef.current : '';
    const formTrigger = formTriggerRef.current ? formTriggerRef.current : '';
    const closeMe = closeMeRef.current ? closeMeRef.current : '';
  ...
  const submForm = (ev) => {
    ...
    const formEl = ev.currentTarget.parentNode
...

          {
            ...
            <form ref={userFormRef} className="logged " action='/users/logout' method="POST">
              <div className="fold">
                ...
                <button className="logout" onClick={loginOut} type="submit" name="logout">Log-Out
                </button>
              </div>
            </form>
          }
