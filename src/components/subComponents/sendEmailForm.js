...
const EmailForm = (props, ref)=>{
    ...
    useImperativeHandle( ref, () => ({
        ...
        get email_error()  { return mailErr.current       },
        get comment()      { return comment.current       },
        get comment_error(){ return comment_error.current },
    }));


    return  (
        <form onSubmit={ (ev)=> props.sendMessage(ev) } method="POST" action='/users/contact/email'  className=" col-11  col-md-6 ">
            ...
            <a  onClick={()=>{props.showForm()}}  className="closeMe foterCl ">×</a><br/>
