...
const SearchBar = (props) => {
  ...
  const hideResults = (hide = false) => (event = null) => {
  ...
  const mobBtn = () => {
    const searchBar = document.querySelector('.wrapper_MbSearch');
    ...
    if (searchBar && searchBar.style.display === 'none')
      ...
      const results = document.querySelector('.search-suggest ul');
  ...
  const getAll = (e) => {
    ...
    const redirectPaths = new RegExp('(?!\/)article|registration|profile|authentication');
    const checkPathName = redirectPaths.test(props.location.pathname);
            ...
            resultsSearch.map((el, index) => {
              return (
                <li key={index}>
                  ...
                  <div>
                    <Link
                      to={`/article/${el._id}`} style={{'cursor': 'pointer'}}
                      onClick={hideResults(true)}
                    >
                      {el.title}
                    </Link>
