    ...
    if (this.props.msg && this.props.msg.loginSuccess) {
      ...
      const element = (
        <div style={wrapperStyle}>
          <div style={alertStyle} className="alert alert-success" role="alert">{this.props.msg.regSuccess} </div>
        </div>
      );
      ...
      return (
        ...
        <form onSubmit={this.submForm} method="POST" className="regForm" action='users/register'>
          <div className="Registration">
            ...
            <div className="alert alert-danger">{this.props.msg.errorCred} </div>}
            <input type="hidden" name='sender' value=''/> <br/>

            <input type="text" onChange={this.valueChanged} name='name' className="username" placeholder="Username"
                   value={this.state.form.username}/><br/>
            {(this.state.msg && this.state.msg.errorUsername) &&
            <div className="alert alert-danger">{this.state.msg.errorUsername} </div>}

            <input
              type="text" onChange={this.valueChanged} name='email' className="email" placeholder="E-mail"
              value={this.state.form.email}
            />
            <br/>
            {(this.state.msg && this.state.msg.emailErr) &&
            <div className="alert alert-danger">{this.state.msg.emailErr} </div>
            }

            <input type="text" onChange={this.valueChanged} name='firstName' className="firstName"
                   placeholder="First Name" value={this.state.form.firstName}/><br/>

            <input type="text" onChange={this.valueChanged} name='lastName' className="lastName" placeholder="Last Name"
                   value={this.state.form.lastName}/><br/>

            <input type="text" onChange={this.valueChanged} name='phone' className="phone" placeholder="Phone Number"
                   value={this.state.form.phone}/><br/><br/>

            <input type="password" onChange={this.valueChanged} name='password' placeholder="Password"
                   className="password" value={this.state.form.password}></input>
            {(this.state.msg && this.state.msg.errorPassword) &&
            <div className="alert alert-danger">{this.state.msg.errorPassword} </div>}

            <input type="password" onChange={this.valueChanged} name='password_confirmation'
                   placeholder="Confirm Password" className="pasConfirm" value={this.state.form.password_confirmation}/>
            {(this.state.msg && this.state.msg.errorPass_confirm) &&
            <div className="alert alert-danger">{this.state.msg.errorPass_confirm} </div>}
            <br/>
            {(this.props.msg && this.props.msg.regSuccess && !this.props.errorCred) &&
            <div className="alert alert-success" role="alert">{this.props.regSuccess} </div>}

            <br/>
            {(this.props.msg && this.props.msg.erCaptcha) &&
            <div className="alert alert-danger">{this.props.msg.erCaptcha} </div>}
            {/* <Recaptcha
